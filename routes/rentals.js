...
router.post('/', auth, async (req, res) => {
    ...
    if(rental) return res.status(400).send(`The movie is already rented by the user '${customer.name}'...`);

    if (movie.numberInStock === 0) return res.status(400).send(`Movie with given Id ${req.body.movieId} is not in stock!`);
